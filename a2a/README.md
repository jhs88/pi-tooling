...
Concurrent workspace hosts serialize registry updates through the private `contexts.json.lock` directory so one host cannot overwrite another host's mappings. The lock is removed after each update. After a hard process crash during an update, remove this lock directory only after confirming that no Pi A2A host using that agent directory is running.
