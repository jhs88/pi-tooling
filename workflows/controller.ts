...
export const MAX_AGENT_CALLS = 3;
export const RUN_SHUTDOWN_TIMEOUT_MS = 8_000;
export const BACKGROUND_RUN_TIMEOUT_MS = 30 * 60 * 1_000;
...
/** Arm a hard deadline so a detached background run cannot live indefinitely. */
export function armBackgroundRunDeadline(
  controller: RunController,
  timeoutMs = BACKGROUND_RUN_TIMEOUT_MS,
) {
  const timer = setTimeout(
    () => controller.abort(`Background workflow exceeded ${timeoutMs} ms`),
