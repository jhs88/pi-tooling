...
export async function createWorkflowResources(
  ...
  const [resources, modelRuntime] = await Promise.all([
    createChildResources({
      cwd,
      projectTrusted,
      ...(variant === "structured"
...
export async function runAgent(
  ...
  const sync = () => {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      const msg = messages[i];
      if (msg.role !== "assistant") continue;
      // Some gateways report a concrete fallback model. Prefer its runtime
      ...
      const reportedModel = responseMatchesSession
        ? options.modelRuntime.getModel(msg.provider, reportedId)
        : undefined;
      if (reportedModel) {
