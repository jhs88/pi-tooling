...
test("RunController reserves calls synchronously and caps global fanout", async () => {
  ...
  let active = 0;
  let peak = 0;
  const tasks = Array.from({ length: MAX_AGENT_CALLS }, (_, index) =>
  ...
  assert.deepEqual(
    ...
    Array.from({ length: MAX_AGENT_CALLS }, (_, i) => i),
  );
  assert.equal(peak, 3);
...
test("background workflow deadline aborts and can be disarmed", async () => {
  const timed = new RunController();
  const clearTimed = armBackgroundRunDeadline(timed, 10);
  await new Promise((resolve) => setTimeout(resolve, 20));
  assert.equal(timed.signal.aborted, true);
  assert.match(String(timed.signal.reason), /Background workflow exceeded 10 ms/);
  clearTimed();

  const disarmed = new RunController();
  const clear = armBackgroundRunDeadline(disarmed, 10);
  clear();
  await new Promise((resolve) => setTimeout(resolve, 20));
  assert.equal(disarmed.signal.aborted, false);
});
